...
const settings = [
  { icon: Buildings, title: "Operating brand", copy: "Set up the brand profile used by campaigns, imports, reports, connections, and team review context.", status: "Brand-scoped", tone: "" },
  ...
  { icon: Bell, title: "Email & webhooks", copy: "Send transactional emails, report delivery, team invitations, lead alerts, and future outbound webhooks from one control surface.", status: "Email-ready", tone: "" },
...
export default function SettingsPage() {
  ...
  const openBrandManager = () => {
    setActive("Operating brand");
    window.dispatchEvent(new CustomEvent("prifyn-open-brand-manager"));
  };
  return <div className="app-content"><header className="app-page-head"><div><span>Workspace governance</span><h1>Settings</h1><p>Shape how your team works, reviews evidence, and receives decisions.</p></div></header><div className="settings-grid">{settings.map(({ icon: Icon, title, copy, status, tone }) => <section className={`surface settings-card ${active === title ? "selected" : ""}`} key={title}><span className="settings-icon"><Icon weight="duotone" /></span><div><h2>{title}</h2><p>{copy}</p>{active === title && <p className="settings-confirmation" role="status">{title === "Operating brand" ? "Brand profile opened. Save the operating brand details used across campaigns and reports." : title === "Email & webhooks" ? "Email is ready for lead alerts, confirmations, team invitations, and report delivery. Webhooks should be added with signing and retry history." : "This control is ready. Changes will become editable when your workspace is connected."}</p>}</div><span className={`status-pill ${tone}`}><CheckCircle weight="fill" /> {status}</span>{title === "Connections" ? <WorkspaceLink className="button button-outline" href="/app/settings/connections">Manage</WorkspaceLink> : title === "Team & access" ? <WorkspaceLink className="button button-outline" href="/app/settings/team">Manage</WorkspaceLink> : title === "Billing & usage" ? <WorkspaceLink className="button button-outline" href="/app/settings/billing">Manage</WorkspaceLink> : title === "Operating brand" ? <button className="button button-outline" type="button" onClick={openBrandManager}>Manage</button> : <button className="button button-outline" type="button" aria-expanded={active === title} onClick={() => setActive(active === title ? null : title)}>{active === title ? "Close" : "Manage"}</button>}</section>)}</div></div>;
}
