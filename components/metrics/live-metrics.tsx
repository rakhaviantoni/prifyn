...
export function LiveReportMetrics() {
  const { summary, loading } = useMetricSummary();
  ...
  return <><section className="surface report-command live-report-command"><div><span className="section-kicker">Live report</span><h2>Imported evidence is ready for decisions.</h2><p>{summary.factCount} normalized facts from {summary.importCount} import batch{summary.importCount === 1 ? "" : "es"}. Derived metrics are calculated from mapped spend, reach, clicks, orders, and revenue.</p></div><WorkspaceLink className="button button-dark" href="/app/settings/imports">Import more</WorkspaceLink></section><section className="surface"><div className="surface-head"><h2>Executive metrics</h2><span>{summary.sourceCount} sources</span></div><div className="metric-grid"><Metric label="Spend" value={formatCurrency(summary.totals.spend_idr ?? 0)} change={`CPC ${formatCurrency(summary.derived.cpc ?? 0)}`} /><Metric label="Impressions" value={formatCompactNumber(summary.totals.impressions ?? 0)} change={`CTR ${summary.derived.ctr === null ? "—" : `${(summary.derived.ctr * 100).toFixed(2)}%`}`} /><Metric label="Revenue" value={formatCurrency(summary.totals.revenue_idr ?? 0)} change={`ROAS ${formatRatio(summary.derived.roas)}`} /><Metric label="Orders / results" value={formatCompactNumber(summary.totals.orders ?? summary.totals.results ?? 0)} change={`CVR ${summary.derived.cvr === null ? "—" : `${(summary.derived.cvr * 100).toFixed(2)}%`}`} /></div></section><section className="surface table-wrap" style={{ marginTop: 18 }}><div className="surface-head"><h2>Source performance</h2><span>Database facts</span></div><table className="data-table"><thead><tr><th>Source</th><th>Spend</th><th>Revenue</th><th>Impressions</th><th>Clicks</th><th>Orders</th><th>ROAS</th></tr></thead><tbody>{summary.bySource.map(row => <tr key={row.source}><td><strong>{row.source}</strong></td><td>{formatCurrency(row.spend)}</td><td>{formatCurrency(row.revenue)}</td><td>{formatCompactNumber(row.impressions)}</td><td>{formatCompactNumber(row.clicks)}</td><td>{formatCompactNumber(row.orders)}</td><td><strong>{formatRatio(row.roas)}</strong></td></tr>)}</tbody></table></section></>;
}
