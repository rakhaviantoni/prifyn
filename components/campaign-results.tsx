...
export function CampaignResults({ report, onReportChange }: { report: string; onReportChange: (value: string) => void }) {
  ...
  const hasLeads = (summary.totals.lead_count ?? 0) > 0;
  ...
  function exportReport() {
    const rows = [
      ...
      `Leads,${summary.totals.lead_count ?? 0}`,
  ...
  return <div className="campaign-results">
    ...
    {summary.hasData ? <section className="results-kpi-grid"><ResultKpi label="Spend" value={formatCurrency(summary.totals.spend_idr ?? 0)} change={`${summary.sourceCount} source${summary.sourceCount === 1 ? "" : "s"}`} tone="neutral" /><ResultKpi label="Reach" value={formatCompactNumber(summary.totals.reach ?? 0)} change="Audience reached" tone="neutral" /><ResultKpi label="Impressions" value={formatCompactNumber(summary.totals.impressions ?? 0)} change="Delivered impressions" tone="neutral" /><ResultKpi label="Leads / results" value={formatCompactNumber(summary.totals.lead_count ?? summary.totals.results ?? summary.totals.orders ?? summary.totals.conversions ?? 0)} change={hasLeads ? "Lead capture" : "Imported objective"} tone="neutral" /><ResultKpi label="ROAS" value={formatRatio(summary.derived.roas)} change={hasRevenue ? "Revenue ÷ spend" : "Add revenue data"} tone={hasRevenue ? "positive" : "neutral"} /></section> : <section className="surface empty-state"><FileArrowUp /><h2>No campaign results yet</h2><p>Upload Meta, TikTok, Google, marketplace, or affiliate exports to populate results.</p><WorkspaceLink className="button button-dark" href="/app/settings/imports">Import report</WorkspaceLink></section>}

    {summary.hasData && <section className="surface results-decision"><span>{hasRevenue && hasClicks ? <Sparkle weight="fill" /> : <Warning weight="fill" />}</span><div><small>{hasRevenue && hasClicks ? "Action available" : "Next data needed"}</small><h2>{hasRevenue && hasClicks ? "Review spend efficiency before scaling." : hasLeads ? "Add order or revenue data to connect leads to business outcome." : hasRevenue ? "Add click data to diagnose conversion quality." : "Add leads, orders, or revenue before making ROI decisions."}</h2><p>{hasRevenue && hasClicks ? `PRIFYN can compare spend ${formatCurrency(summary.totals.spend_idr ?? 0)}, clicks ${formatCompactNumber(summary.totals.clicks ?? 0)}, revenue ${formatCurrency(summary.totals.revenue_idr ?? 0)}, and ROAS ${formatRatio(summary.derived.roas)}.` : hasLeads ? `PRIFYN found ${formatCompactNumber(summary.totals.lead_count ?? 0)} captured leads. Import customer/order/revenue data next to see which leads became customers.` : hasRevenue ? "Revenue is available, but click or landing data is missing, so PRIFYN cannot identify where users drop yet." : "The current import measures delivery and awareness. ROI, ROAS, CAC, and journey recommendations need leads, orders, or revenue."}</p><div><b>{summary.importCount} import{summary.importCount === 1 ? "" : "s"}</b><b>{hasClicks ? "Clicks available" : "Clicks missing"}</b><b>{hasLeads ? "Leads available" : "Leads missing"}</b><b>{hasRevenue ? "Revenue available" : "Revenue missing"}</b></div></div><WorkspaceLink className="button button-dark" href={hasRevenue && hasClicks ? "/app/reports" : "/app/settings/imports"}>{hasRevenue && hasClicks ? "Open reports" : "Import outcome data"} <ArrowRight /></WorkspaceLink></section>}

    <section className="surface results-report"><div className="results-report-head"><div><small>Report view</small><h2>{report}</h2></div><div className="page-tabs results-tabs" role="tablist" aria-label="Campaign result views">{reportTypes.map(item => <button type="button" role="tab" aria-selected={report === item} className={report === item ? "active" : ""} onClick={() => onReportChange(item)} key={item}>{item}</button>)}</div></div>{report === "Performance" && <PerformanceReport hasRevenue={hasRevenue} hasClicks={hasClicks} />}{report === "Audience" && <AudienceReport />}{report === "Location" && <LocationReport />}{report === "Creative" && <CreativeReport hasCreativeRows={hasCreativeRows} />}{report === "User journey" && <JourneyReport hasClicks={hasClicks} hasOrders={hasOrders || hasLeads} hasRevenue={hasRevenue} />}</section>
