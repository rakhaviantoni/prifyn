...
export function ConnectionsCenter() {
  ...
  return <div className="connections-center">
    ...
    <section className="surface connections-workspace">
      ...
      <div className="connection-card-grid">{visible.map(provider => {
        ...
        return <article className="connection-card" key={provider.id}>
          ...
          <div className="connection-account"><span>{connected ? <CheckCircle weight="fill" /> : <Info weight="fill" />}</span><div><strong>{connected ? liveAccount?.displayName ?? "Authorized workspace source" : "No channel account connected"}</strong><small>{connected ? `Updated ${liveAccount ? new Date(liveAccount.lastDiscoveredAt).toLocaleString() : "recently"}` : provider.configured ? "Ready to connect with owner permission" : "Setup needed before connection"}</small></div></div>
    ...
    {selected && <div className="dialog-backdrop" role="presentation" onMouseDown={() => setSelected(null)}><section className="dialog-card connection-dialog" role="dialog" aria-modal="true" aria-labelledby="connection-dialog-title" onMouseDown={event => event.stopPropagation()}><button className="dialog-close" type="button" aria-label="Close" onClick={() => setSelected(null)}><X /></button><span className={`connection-provider-mark provider-${selected.id}`}><ProviderLogo provider={selected} /></span><h2 id="connection-dialog-title">{selected.label}</h2><p>{selected.configured ? "Connect the channel account you want PRIFYN to use for reporting, tracking, and campaign review." : "This channel is not ready to connect yet. Use manual exports until connection setup is available."}</p><div className="connection-dialog-facts"><div><span>Access type</span><strong>{selected.auth}</strong></div><div><span>Status</span><strong>{selected.configured ? "Ready to connect" : "Manual import for now"}</strong></div><div><span>Can support</span><strong>{selected.capabilities.length} workflows</strong></div></div>{liveConnections?.find(item => item.provider === selected.id)?.accounts.length ? <div className="connection-account-picker"><strong>Available accounts</strong>{liveConnections.find(item => item.provider === selected.id)!.accounts.map(account => <article key={account.id}><div><b>{account.displayName ?? account.id}</b><small>{account.status} · {account.id}</small></div><button type="button" className="button button-outline" onClick={() => assignAccount(account.id)}>Use for this brand</button></article>)}</div> : null}<div className="connection-dialog-scope">{selected.capabilities.map(item => <span key={item}><CheckCircle />{item}</span>)}</div><aside><Info weight="fill" /><p>Signing in only opens PRIFYN. Pulling Ads, Analytics, YouTube, or commerce data requires connecting each channel here or importing its export file.</p></aside><div className="dialog-actions"><button type="button" className="button button-outline" onClick={() => setSelected(null)}>Close</button>{selected.id === "google" && selected.configured && <button type="button" className="button button-dark" onClick={() => primaryAction(selected)}>Connect Google Ads <ArrowRight /></button>}</div></section></div>}
