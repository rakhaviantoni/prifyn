...
export function Copilot() {
  ...
  return <section className="surface chat-surface"><div className="copilot-context"><span>Answering for</span><strong>{brand === "Operating brand" ? "Unnamed brand profile" : brand}</strong><small>{contextLine(summary)}</small></div>{messages.length === 0 ? <div className="chat-empty"><div><span className="spark-large"><Sparkle weight="fill" /></span><h2>Ask PRIFYN about your growth.</h2><p>Every answer is scoped to the selected brand, reporting period, available data, and your permissions. If data is missing, PRIFYN will say what to connect or import next.</p><div className="prompt-grid">{prompts.map(prompt => <button key={prompt} onClick={() => ask(prompt)}>{prompt}</button>)}</div></div></div> : <div className="chat-messages">{messages.map((message, index) => <div className={`chat-message ${message.role}`} key={`${message.role}-${index}`}>{message.role === "assistant" && <strong>PRIFYN analysis</strong>}{message.content}{message.why && <small style={{ display: "block", marginTop: 8, color: "#69736e" }}>{message.why}</small>}</div>)}{loading && <div className="chat-message assistant"><strong>PRIFYN analysis</strong>Reviewing brand data…</div>}</div>}<form className="chat-composer" onSubmit={submit}><input aria-label="Ask PRIFYN" value={value} onChange={event => setValue(event.target.value)} placeholder="Ask about revenue, campaigns, creators, or next actions…" /><button type="submit" disabled={!value.trim() || loading} aria-label="Send question"><ArrowUp weight="bold" /></button></form></section>;
}
