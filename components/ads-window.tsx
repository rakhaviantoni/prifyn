...
type DeliveryStatus = "In review" | "Scheduled" | "Active" | "Learning" | "Learning limited" | "Rejected" | "Completed";

const platforms = ["Meta", "Google", "TikTok", "Tokopedia", "Shopee"];
const reportTypes = ["Performance", "Audience", "Location", "Creative", "User journey"];
const ctaOptions = ["Shop Now", "Learn More", "Sign Up", "Download", "Contact Us", "Book Now", "Apply Now", "Send Message", "Subscribe", "Get Quote", "Listen Now", "Watch More", "See Menu", "See Details"];
...
const statusCopy: Record<DeliveryStatus, string> = {
  ...
  Rejected: "A policy issue is preventing this campaign from running.",
...
export function AdsWindow() {
  ...
  return <div className="app-content workflow-page">
    ...
    {stage === "channels" && <div className="workflow-stack"><section className="surface workflow-card"><div className="workflow-card-head"><span><LinkSimple weight="duotone" /></span><div><small>Channel & account</small><h2>Choose where—and under which account—the ad will run</h2><p>Connections are simulated in this preview. Publishing integrations remain pending third-party validation.</p></div></div><fieldset className="field platform-picker channel-picker"><legend>Selected channels</legend>{platforms.map(platform => <label key={platform} className={selectedPlatforms.includes(platform) ? "selected" : ""}><input type="checkbox" checked={selectedPlatforms.includes(platform)} onChange={() => togglePlatform(platform)} /><span>{platform}</span></label>)}</fieldset><div className="account-connection-list">{platforms.map(platform => { const selected = selectedPlatforms.includes(platform); const isConnected = connected.includes(platform); return <article key={platform} className={!selected ? "disabled" : ""}><div className="channel-account-head"><span className="channel-logo">{platform.slice(0, 2).toUpperCase()}</span><div><strong>{platform}</strong><small>{!selected ? "Not selected" : isConnected ? "Connected · choose the publishing identity" : "Account connection required"}</small></div><button type="button" className={`button ${isConnected ? "button-outline" : "button-dark"}`} disabled={!selected} onClick={() => { setConnected(current => isConnected ? current.filter(item => item !== platform) : [...current, platform]); setChannelsSaved(false); }}>{isConnected ? <><CheckCircle weight="fill" /> Connected</> : "Connect account"}</button></div>{selected && isConnected && <label className="field"><span>Publishing account</span><select value={selectedAccounts[platform]} onChange={event => setSelectedAccounts(current => ({ ...current, [platform]: event.target.value }))}>{accounts[platform].map(account => <option key={account}>{account}</option>)}</select></label>}</article>; })}</div></section><section className="surface channel-ready-card"><div><CheckCircle weight="fill" /><span><strong>{channelsReady ? "All selected channels have an account." : "Connect every selected channel to continue."}</strong>{selectedPlatforms.length} selected · {selectedPlatforms.filter(item => connected.includes(item)).length} ready</span></div><button type="button" className="button button-dark" disabled={!channelsReady} onClick={saveChannels}>Review campaign <ArrowRight /></button></section></div>}
