...
export function AdminBusinessManager({ metrics, leads, imports }: {
  metrics: { users: number; workspaces: number; operatingBrands: number; leads: number; imports: number; webhooks: number };
  leads: Lead[];
  imports: ImportItem[];
}) {
  ...
  const metricCards = [
    { label: "Users", value: metrics.users, icon: UsersThree },
    { label: "Workspaces", value: metrics.workspaces, icon: Buildings },
    { label: "Operating brands", value: metrics.operatingBrands, icon: Sparkle },
    { label: "Leads", value: metrics.leads, icon: Funnel },
    { label: "Imports", value: metrics.imports, icon: Database },
    { label: "Webhooks", value: metrics.webhooks, icon: PlugsConnected },
  ];

  return <div className="admin-page"><header className="admin-hero"><div><span>PRIFYN Business Manager</span><h1>Operate every client from first request to reporting.</h1><p>Track prospects, client accounts, imports, reporting readiness, and the next operational handoff in one PRIFYN-side workspace.</p></div><a className="button button-light" href="/app">Open client workspace <ArrowRight /></a></header><section className="admin-metrics">{metricCards.map(({ label, value, icon: Icon }) => <article className="surface" key={label}><Icon weight="duotone" /><span><strong>{value}</strong>{label}</span></article>)}</section><section className="surface admin-flow"><div><span>Client order flow</span><strong>Intake → Meeting / Online Request → Approval → Media Plan → Internal Brief → Ad Operation → Running → Reporting → Client Review</strong><p>Use this flow to keep each lead clear: who owns it, what stage it is in, and what PRIFYN should do next.</p></div></section><div className="admin-grid"><section className="surface admin-table"><div className="surface-head"><div><h2>Lead & order pipeline</h2><span>{rows.length} recent lead{rows.length === 1 ? "" : "s"}</span></div></div>{rows.length ? <div className="admin-lead-list">{rows.map(lead => <article key={lead.id} className={selected?.id === lead.id ? "selected" : ""}><button type="button" onClick={() => setSelected(lead)}><strong>{lead.company}</strong><span>{lead.contact} · {lead.email}</span><small>{lead.channel || lead.source} · {lead.spend || "Spend unknown"}</small></button><select value={lead.status} aria-label={`Stage for ${lead.company}`} onChange={event => updateStage(lead.id, event.target.value)}>{adminOrderStages.map(stage => <option value={stage} key={stage}>{stageLabel(stage)}</option>)}</select></article>)}</div> : <div className="empty-state compact"><Funnel /><h2>No leads yet</h2><p>Book appointment and Apply online submissions will appear here.</p></div>}</section><aside className="admin-side">{selected && <section className="surface admin-detail"><span className="section-kicker">Selected account</span><h2>{selected.company}</h2><p>{selected.problem || "No problem statement yet."}</p><div className="admin-detail-grid"><div><span>Contact</span><strong>{selected.contact}</strong><small>{selected.email}</small></div><div><span>Stage</span><strong>{stageLabel(selected.status)}</strong></div><div><span>Workspace</span><strong>{selected.workspace}</strong><small>{selected.brand}</small></div><div><span>Timing</span><strong>{selected.urgency || "Not set"}</strong></div></div><div className="admin-actions"><a className="button button-dark" href={`mailto:${selected.email}`}><EnvelopeSimple /> Email client</a><button className="button button-outline" type="button" onClick={() => updateStage(selected.id, "approval_requested")}>Send for approval</button><button className="button button-outline" type="button" onClick={() => updateStage(selected.id, "internal_brief")}>Create internal brief</button></div></section>}<section className="surface admin-detail"><div className="surface-head"><h2><ChartLineUp /> Recent imports</h2></div><div className="campaign-mini-list">{imports.length ? imports.map(item => <div className="campaign-mini" key={item.id}><i /><div><strong>{item.sourceType}</strong><span>{item.brand} · {item.acceptedRows}/{item.totalRows} rows</span></div><b>{item.status}</b></div>) : <div className="campaign-mini"><i className="warning" /><div><strong>No imports yet</strong><span>Client exports will appear after upload.</span></div><b>Waiting</b></div>}</div></section></aside></div>{notice && <div className="toast" role="status"><CheckCircle weight="fill" />{notice}</div>}</div>;
}
