...
export function CampaignWorkspace({ initialCreating = false, initialCampaigns = [] }: { initialCreating?: boolean; initialCampaigns?: Campaign[] }) {
  ...
  function createCampaign(form: FormData) {
    ...
    if (!name || !end) return;
    const draft: Campaign = { name, status: "Draft", objective, owner: "Workspace", creators: "Not linked", revenue: "Rp 0", roas: "Needs revenue", end, note: note || "New draft campaign. Add a brief, channel plan, and tracking method before launch.", nextAction: "Complete brief", tracking: "Not configured", source: "campaign" };
