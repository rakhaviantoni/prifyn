...
export function AppShell({ children, currentUser }: { children: ReactNode; currentUser?: ShellUser }) {
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      setDark(enabled);
      if (storedBrand) setActiveBrand({ initials: initialsFrom(storedBrand), name: storedBrand, detail: "Operating brand" });
    });
  }, []);
  useEffect(() => {
  ...
  return <div className="app-layout">
    <aside id="workspace-navigation" className={`app-sidebar ${mobileOpen ? "mobile-open" : ""}`} aria-label="Workspace menu">
      ...
      <div className="workspace-switcher"><button className="workspace-button" type="button" aria-expanded={brandOpen} onClick={() => setBrandOpen(value => !value)}><b>{activeBrand.initials}</b><div><strong>{activeBrand.name}</strong><span>{activeBrand.detail}</span></div><CaretDown className={brandOpen ? "rotated" : ""} /></button>{brandOpen && <div className="workspace-menu" role="menu" aria-label="Brand menu"><button type="button" role="menuitem" className="active" onClick={() => setBrandOpen(false)}><b>{activeBrand.initials}</b><span><strong>{activeBrand.name}</strong><small>{activeBrand.detail}</small></span><CheckCircle weight="fill" /></button><WorkspaceLink href="/app/settings" role="menuitem" onClick={() => setBrandOpen(false)}><b>+</b><span><strong>Add or edit brands</strong><small>Manage operating brand slots</small></span></WorkspaceLink><WorkspaceLink href="/app/settings/team" role="menuitem" onClick={() => setBrandOpen(false)}><b>TM</b><span><strong>Team access</strong><small>Invite members and assign roles</small></span></WorkspaceLink></div>}</div>
