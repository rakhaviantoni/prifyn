...
type InterviewSummary = { headline: string; why: string[]; risks: string[]; recommendation: string; confidence: number };
type Candidate = { id: string; profileId?: string; name: string; handle: string; initials: string; fit: number; rate: string; summary: string; interview: InterviewSummary };
...
const seedCandidates = Object.fromEntries(creatorProfiles.map(creator => [creator.id, {
  id: creator.id,
  ...
  fit: creator.fit,
  rate: creator.rate,
  summary: `${creator.strengths[0]} · ${creator.platforms[0]}`,
  interview: {
    headline: `${creator.summary} Based on similar campaign signals, PRIFYN expects ${creator.fit >= 88 ? "above-average" : "review-required"} performance for this brief.`,
    why: [creator.niches[0] ? `${creator.niches[0]} niche fit` : "Relevant content niche", `${creator.location} location signal`, `${creator.engagement} engagement`, creator.strengths[0]],
    risks: [creator.risk, creator.verification === "Review" ? "Verification still needs human review" : "Confirm final availability before selection"],
    recommendation: creator.fit >= 88 ? "Shortlist and confirm schedule, usage rights, and tracking requirements." : "Keep in review until audience and availability evidence improves.",
    confidence: Math.min(96, Math.max(72, creator.fit - 2)),
  },
...
export function TalentPipeline() {
  ...
  function addCandidate(form: FormData) {
    ...
    setCandidates(current => ({ ...current, [id]: { id, name, handle, initials, fit: 0, rate: "Rate pending", summary: "Manual candidate · AI analysis pending", interview: { headline: "AI Interview Summary will be generated after profile, portfolio, and application evidence are reviewed.", why: ["Manual candidate added"], risks: ["Evidence not reviewed yet"], recommendation: "Request profile links and rate card before shortlisting.", confidence: 0 } } }));
  ...
  return <div className="app-content talent-pipeline-page">
    ...
    <section className="talent-board">{stages.map(stage => <div className="talent-column" key={stage}><header><span>{stage}</span><b>{pipeline[stage].length}</b></header>{pipeline[stage].map(id => { const candidate = candidates[id]; return <article className="talent-card surface" key={id}>
      ...
      <p>{candidate.summary}</p>{["Applied", "Shortlisted"].includes(stage) && <section className="candidate-interview-summary"><span><Sparkle weight="fill" /> AI Interview Summary</span><p>{candidate.interview.headline}</p><div><strong>Why fit</strong>{candidate.interview.why.slice(0, 3).map(item => <b key={item}>{item}</b>)}</div><aside><Warning weight="fill" /><small>{candidate.interview.risks[0]}</small></aside></section>}{(notes[id] || messages[id]) && <div className="candidate-activity">{notes[id] && <span><NotePencil /> {notes[id]} note{notes[id] > 1 ? "s" : ""}</span>}{messages[id] && <span><ChatCircleDots /> {messages[id]} sent</span>}</div>}
    ...
    {dialog?.type === "review" && <div className="dialog-backdrop" onMouseDown={() => setDialog(null)}><section className="dialog-card application-review-dialog" role="dialog" aria-modal="true" aria-labelledby="application-review-title" onMouseDown={event => event.stopPropagation()}><button className="dialog-close" type="button" aria-label="Close" onClick={() => setDialog(null)}><X /></button><span className="section-kicker">Application review</span><h2 id="application-review-title">{candidates[dialog.id].name}</h2><div className="review-facts"><div><span>Proposed rate</span><strong>{candidates[dialog.id].rate}</strong></div><div><span>Campaign fit</span><strong>{candidates[dialog.id].fit}%</strong></div><div><span>Confidence</span><strong>{candidates[dialog.id].interview.confidence}%</strong></div></div><section className="ai-interview-dialog-card"><span><Sparkle weight="fill" /> AI Interview Summary</span><p>{candidates[dialog.id].interview.headline}</p><div className="ai-interview-columns"><div><strong>Why this creator fits</strong>{candidates[dialog.id].interview.why.map(item => <b key={item}><CheckCircle weight="fill" />{item}</b>)}</div><div><strong>Potential risks</strong>{candidates[dialog.id].interview.risks.map(item => <b key={item}><Warning weight="fill" />{item}</b>)}</div></div></section><blockquote>“I can translate the brief into a clear business story and deliver the first cut within five working days.”</blockquote><div className="review-confidence"><Sparkle weight="fill" /><span><strong>Recommended action</strong>{candidates[dialog.id].interview.recommendation}</span></div><div className="review-action-grid"><button className="button button-outline" type="button" onClick={() => { archive(dialog.id); setDialog(null); }}>Reject</button><button className="button button-outline" type="button" onClick={() => setDialog({ type: "message", id: dialog.id })}>Request information</button><button className="button button-dark" type="button" onClick={() => { move(dialog.id, "Selected"); setDialog(null); }}>Select creator</button></div></section></div>}
