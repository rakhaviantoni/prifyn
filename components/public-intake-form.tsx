...
export function PublicIntakeForm({ type }: { type: IntakeType }) {
  ...
  async function submit(form: FormData) {
    ...
    const preferredTime = isAppointment
      ? customTime.trim() || (selectedSlotValue ? `${selectedSlotValue.label} walkthrough · ${selectedSlotValue.startTime}–${selectedSlotValue.endTime} ${selectedSlotValue.timezone}` : undefined)
      : undefined;
    setSubmitting(true);
    setError(null);
    try {
  ...
  return <section className="surface public-intake-card">
    ...
    <form className="public-intake-form" action={submit}>
      ...
      {isAppointment && <fieldset className="intake-fieldset"><legend>Preferred walkthrough time</legend><div className="appointment-slots">{slots.map(slot => <button className={selectedSlot === slot.id && !customTime ? "selected" : ""} type="button" key={slot.id} onClick={() => { setSelectedSlot(slot.id); setCustomTime(""); }}><CalendarBlank weight="duotone" /><span><strong>{slot.label}</strong><small><Clock />{slot.startTime}–{slot.endTime}</small><em>{slot.note || slot.timezone}</em></span></button>)}</div><label className="field custom-time-field"><span>Need another time?</span><input value={customTime} onChange={event => setCustomTime(event.target.value)} placeholder="Example: Tuesday or Thursday afternoon WIB" /></label></fieldset>}
