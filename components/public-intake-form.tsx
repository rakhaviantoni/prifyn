...
export function PublicIntakeForm({ type }: { type: IntakeType }) {
  ...
  async function submit(form: FormData) {
    ...
    const preferredTime = isAppointment
      ? customTime.trim() || (selectedSlotValue ? `${selectedSlotValue.label} walkthrough · ${formatSlotDate(selectedSlotValue.availableDate)} · ${selectedSlotValue.startTime}–${selectedSlotValue.endTime} ${selectedSlotValue.timezone}` : undefined)
      : undefined;
    setSubmitting(true);
    setError(null);
    try {
